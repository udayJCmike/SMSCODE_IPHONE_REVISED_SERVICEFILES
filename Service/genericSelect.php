    ...
    case 'providerSelect':
    {
        ...
        if($query11 = mysql_query($sql11))
        {
            ...
            if($flag == '1')
            {
                ...
                {
                    $record11 =mysql_fetch_array($query11);
                    $sql12 ="SELECT admin_username,admin_firstname,admin_mobile,admin_email,secondary_email FROM admin_log_table WHERE admin_username='".$record11['Provider_name']."'";

                    $query12 = mysql_query($sql12);
                    $row		= mysql_fetch_object($query12);

                    $json 		.= '{ "serviceresponse" : { "servicename" : "Select Providerdetail", "success" : "Yes", "adminusername" : "'.$row->admin_username.'", "adminfirstname" : "'.$row->admin_firstname.'", "adminmobile" : "'.$row->admin_mobile.'", "adminemail" : "'.$row->admin_email.'","adminsecondaryemail" : "'.$row->secondary_email.'","message" : "1" } }';
        ...
        if($query11 = mysql_query($sql))
        {
            ...
            if($flag == '1')
            {
                ...
                for($i=0;$i<mysql_num_rows($query11);$i++)
                {
                    ...
                    $meddetail=$row->medical_details;

                    $snip = str_replace(array( "\r\n", "\n", "\t", "\r"), '',$meddetail);


                    $json 		.= '{ "serviceresponse" : { "servicename" : "Select Participant", "success" : "Yes", "idnumber" : "'.$row->id.'" ,"firstname" : "'.$row->fname.'" ,"username" : "'.$row->username.'" , "mobilenum" : "'.$row->mobile_num.'", "email" : "'.$row->email_id.'", "gender" : "'.$row->gender.'", "city" : "'.$row->city.'", "education" : "'.$row->education.'", "medical" : "'.$snip.'", "time1" : "'.$row->time1.'","time1format" : "'.$row->time1_am_pm.'","time2" : "'.$row->time2.'","time2format" : "'.$row->time2_am_pm.'", "time3" : "'.$row->time3.'","time3format" : "'.$row->time3_am_pm.'", "providername" : "'.$row->Provider_name.'", "group" : "'.$row->group_name.'", "age" : "'.$row->age.'" ,"messagestream" : "'.$row->message.'"} }';
